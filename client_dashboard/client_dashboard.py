"""
Web Dashboard для TikTok Analytics Bot (Клиентская версия с анонимизацией)
Запуск: python client_dashboard.py
...
app = FastAPI(
    title="TikTok Analytics Dashboard (Client)",
    description="Web интерфейс для просмотра аналитики (клиентская версия)",
...
# Учетные данные для клиентского дашборда
VALID_CREDENTIALS = {
    "client": "clientpass"
}
...
class UserAnonymizer:
    """Класс для анонимизации пользователей с сохранением консистентности"""
    
    def __init__(self):
        self._user_mapping = {}
        self._counter = 1
    
    def anonymize(self, telegram_user: str) -> str:
        """
        Анонимизировать имя пользователя
        
        Args:
            telegram_user: Оригинальное имя пользователя
            
        Returns:
            Анонимизированное имя (например, "Менеджер 1")
        """
        if not telegram_user or telegram_user.strip() == "":
            return ""
        
        # Если пользователь уже был анонимизирован, возвращаем тот же псевдоним
        if telegram_user in self._user_mapping:
            return self._user_mapping[telegram_user]
        
        # Создаём новый псевдоним
        anonymous_name = f"Менеджер {self._counter}"
        self._user_mapping[telegram_user] = anonymous_name
        self._counter += 1
        
        return anonymous_name
    
    def anonymize_profiles(self, profiles: List[Dict]) -> List[Dict]:
        """
        Анонимизировать список профилей
        
        Args:
            profiles: Список профилей с данными
            
        Returns:
            Список профилей с анонимизированными именами пользователей
        """
        anonymized_profiles = []
        
        for profile in profiles:
            # Создаём копию профиля
            anonymized_profile = profile.copy()
            
            # Анонимизируем telegram_user
            original_user = profile.get('telegram_user', '')
            anonymized_profile['telegram_user'] = self.anonymize(original_user)
...
# Глобальный экземпляр анонимизатора
...
async def dashboard_page(
    ...
    try:
        # Получаем данные и анонимизируем
        summary = sheets_reader.read_all_platforms()
        summary = anonymizer.anonymize_profiles(summary)
...
async def get_growth_stats(
    ...
    try:
        ...
        # Получаем статистику
        stats = history.get_stats_by_period(start_date_str, end_date_str)
...
async def get_platforms_stats(username: str = Depends(verify_credentials)):
    try:
        # Получаем данные и анонимизируем
        summary = sheets_reader.read_all_platforms()
        summary = anonymizer.anonymize_profiles(summary)
    ...
    try:
        ...
        for profile in top_profiles:
            result.append({
                ...
                "telegram_user": profile.get("telegram_user", ""),  # Уже анонимизировано
...
async def get_top_users_by_views(
    ...
    try:
        ...
        profiles = anonymizer.anonymize_profiles(profiles)
        ...
        for profile in profiles:
            ...
            if telegram_user not in user_stats:
                user_stats[telegram_user] = {
                    "telegram_user": telegram_user,  # Уже анонимизировано
                    "total_views": 0,
                    "profiles_count": 0
                }
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("🚀 Запуск Client Web Dashboard (с анонимизацией)...")
    print("=" * 60)
    print(f"URL: http://localhost:8003")
    print(f"Логин: client")
    print(f"Пароль: clientpass")
    print("=" * 60)
    print("ℹ️  Все имена пользователей анонимизированы")
    print("=" * 60)
